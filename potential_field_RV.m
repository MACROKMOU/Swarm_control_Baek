function [Z_re,psi_d_re,point_vec_re,vertex_1] = potential_field_RV(ship,target,obs_1,vel_1,err_1,obs_2,vel_2,err_2,obs_3,vel_3,err_3)
...
for t=1:dt:299    %%% field 계산
    ii=1;
    for tt=1:dtt:299
        z_1 = a/(1+exp(b*(-(ii-vertex_1(1))*cosd(obs1_angle_1)+(i-vertex_1(2))*sind(obs1_angle_1))))-a/(1+exp(b*(-(ii-vertex_1(1))*cosd(obs1_angle_2)+(i-vertex_1(2))*sind(obs1_angle_2))));
       if z_1 < 0 || abs(err_1) < 15
           z_1 = 0;
       end
%        Z_1(ii,i) = z_1;
        
       z_2 = a/(1+exp(b*(-(ii-vertex_2(1))*cosd(obs2_angle_1)+(i-vertex_2(2))*sind(obs2_angle_1))))-a/(1+exp(b*(-(ii-vertex_2(1))*cosd(obs2_angle_2)+(i-vertex_2(2))*sind(obs2_angle_2))));
       if z_2 < 0 || abs(err_2) < 15
           z_2 = 0;
       end
%        Z_2(ii,i) = z_2;
       z_3 = a/(1+exp(b*(-(ii-vertex_3(1))*cosd(obs3_angle_1)+(i-vertex_3(2))*sind(obs3_angle_1))))-a/(1+exp(b*(-(ii-vertex_3(1))*cosd(obs3_angle_2)+(i-vertex_3(2))*sind(obs3_angle_2))));
       if z_3 < 0 || abs(err_3) < 15
           z_3 = 0;
       end
%        Z_3(ii,i) = z_3;
       z_4 = 1*tanh(35*(1/(real(sqrt((ship(1)-i)^2+(ship(2)-ii)^2)))))-1*tanh(35*(1/(real(sqrt((target(1)-i)^2+(target(2)-ii)^2)))));
       A = [z_1 z_2 z_3 z_4];
